...
xmlhttp5.onreadystatechange = function() {
	if (this.readyState == 4 && this.status == 200) {
		...
		restoreButtons.forEach(function(button) {
			button.addEventListener('click', function() {
				appendAlert('This is a premium feature and is not supported in the free version. To restore a user, please contact the administrator', 'warning');
			});
		});
	...
	if (this.readyState == 4 && this.status == 200) {
		...
		areas.forEach(function(area) {
			...
			newElement.innerHTML = `
				...
				<td>
					<button type="submit" class="btn btn-primary update-area">Update</button>
					<button type="submit" class="btn btn-danger delete-area">Delete</button>
				</td>
			`;
